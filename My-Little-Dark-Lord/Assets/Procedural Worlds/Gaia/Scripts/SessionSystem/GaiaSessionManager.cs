...
namespace Gaia
{
    ...
    public class GaiaSessionManager : MonoBehaviour
    {
        ...
        public CollisionMaskCache m_collisionMaskCache = new CollisionMaskCache();
        ...
        private void OnSceneSaved(Scene scene)
        {
            ...
            {
                m_collisionMaskCache.WriteCacheToDisk();
            }
        }
        ...
        private void OnHeightmapChanged(Terrain terrain, RectInt heightRegion, bool synched)
        {
            ...
            //Look for all stampers in the scene and update their heightmasks in case they are affected by the change
            bool stampersUpdated = false;
            Stamper[] stampers = FindObjectsOfType<Stamper>();
            int length = stampers.Length;
            for (int i = 0; i < length; i++)
            {
                stampers[i].UpdateMinMaxHeight();
                stampersUpdated = true;
            }
            //if at least one stamper was updated, all spawners will be as well, otherwise we have to iterate those as well.
            if (!stampersUpdated)
            {
        ...
        public static GaiaSessionManager GetSessionManager(bool pickupExistingTerrain = false)
        {
            ...
            if (mgrObj == null)
            {
                ...
                if (m_gaiaSettings != null)
                {
                    sessionMgr.SetSeaLevel(m_gaiaSettings.m_currentDefaults.m_seaLevel);
                }
        ...
        public void GetWorldMinMax(ref float minWorldHeight, ref float maxWorldHeight)
        {
            ...
            foreach (Terrain t in Terrain.activeTerrains)
            {
                ...
                GetTerrainMinMax(t, ref newMin, ref newMax, false);
...
#if UNITY_EDITOR
            string path = GaiaDirectories.GetDataDirectory();
        ...
        public float GetSeaLevel()
        {
            ...
            if (m_gaiaSettings != null)
            {
                return m_gaiaSettings.m_currentDefaults.m_seaLevel;
            }
...
#region Terrain Min Max Cache
        ...
        private float GetMinMaxTerrainHeight(Terrain terrain, ref float min, ref float max)
        {
            ...
            {
                for (int z = 0; z <= terrain.terrainData.heightmapWidth; z++)
                {

...
namespace Gaia
{
    ...
    public class ImageMaskListEditor : PWEditor, IPWEditor
    {
        ...
        public static float OnElementHeight(int index, ImageMask imageMask)
        {
            ...
            float height = 0;
            //5 first basic mask fields if in first position, 6 if in second
            if (index == 0)
            {
            ...
            switch (imageMask.m_operation)
            {
                ...
                case ImageMaskOperation.CollisionMask:
                    ...
                    return height + EditorGUIUtility.singleLineHeight * (imageMask.m_collisionMasks.Length+4);
        ...
        public static void DrawMaskListElement(Rect rect, int index, ImageMask imageMask, ref CollisionMask[] collisionMaskListBeingDrawn, EditorUtils m_editorUtils, Terrain currentTerrain, bool isStampOperation)
        {
            int oldIndent = EditorGUI.indentLevel;
            EditorGUI.indentLevel = 0;
            ...
            switch (imageMask.m_operation)
            {
                ...
                case ImageMaskOperation.TerrainTexture:
                    ...
                    int[] prototypeValueArray = Enumerable
                                        ...
                                        .ToArray();
                    imageMask.m_textureLayerId = EditorGUI.IntPopup(rect, m_editorUtils.GetContent("MaskTextureLayer").text, imageMask.m_textureLayerId, currentTerrain.terrainData.terrainLayers.Select(x => x.diffuseTexture.name).ToArray(), prototypeValueArray);
                    break;
                default:
                    break;
            }

...
namespace Gaia
{
    ...
    public class GaiaResourceEditor : PWEditor, IPWEditor
    {
        ...
        public static void DrawGeNaSpawnerPrototype(ResourceProtoGeNaSpawner resourceProtoGeNaSpawner, EditorUtils editorUtils, bool showHelp)
        {
            resourceProtoGeNaSpawner.m_name = editorUtils.TextField("GeNaProtoName", resourceProtoGeNaSpawner.m_name, showHelp);
            EditorGUI.indentLevel++;
            resourceProtoGeNaSpawner.m_instancesFoldOut = editorUtils.Foldout("GeNaProtoInstances", resourceProtoGeNaSpawner.m_instancesFoldOut, showHelp);
            //Iterate through instances
            if (resourceProtoGeNaSpawner.m_instancesFoldOut)
            {
                EditorGUI.indentLevel++;
                for (int i = 0; i < resourceProtoGeNaSpawner.m_instances.Length; i++)
                {
                    var instance = resourceProtoGeNaSpawner.m_instances[i];
                    instance.m_foldedOut = editorUtils.Foldout(instance.m_foldedOut, new GUIContent(instance.m_name), showHelp);
                    if (instance.m_foldedOut)
                    {
                        EditorGUI.indentLevel++;
                        instance.m_name = editorUtils.TextField("GeNaProtoName", instance.m_name, showHelp);

                        GameObject oldPrefab = instance.m_spawnerPrefab;

                        if (instance.m_invalidPrefabSupplied)
                        {
                            EditorGUILayout.HelpBox(editorUtils.GetTextValue("GeNaProtoNoGeNaSpawner"), MessageType.Error);
                        }

                        instance.m_spawnerPrefab = (GameObject)editorUtils.ObjectField("GeNaProtoSpawnerPrefab", instance.m_spawnerPrefab, typeof(GameObject), false, showHelp);

                        //New Prefab submitted - check if it actually contains a GeNa spawner
                        if (oldPrefab != instance.m_spawnerPrefab)
                        {
                            //if (instance.m_spawnerPrefab.GetComponent<GeNa.Spawner>() != null)
                            //{
                        ...
                        //instance.m_mobilePrefab = (GameObject)editorUtils.ObjectField("GameObjectProtoInstanceMobile", instance.m_mobilePrefab, typeof(GameObject), false, showHelp);
                        instance.m_minSpawnerRuns = editorUtils.IntField("GeNaProtoSpawnerMinSpawns", instance.m_minSpawnerRuns, showHelp);
                        instance.m_maxSpawnerRuns = editorUtils.IntField("GeNaProtoSpawnerMaxSpawns", instance.m_maxSpawnerRuns, showHelp);
                        instance.m_failureRate = editorUtils.Slider("GeNaProtoSpawnerFailureRate", instance.m_failureRate, 0, 1, showHelp);
                        editorUtils.SliderRange("GameObjectProtoInstanceSpawnOffsetX", ref instance.m_minSpawnOffsetX, ref instance.m_maxSpawnOffsetX, -100, 100, showHelp);
                        //editorUtils.SliderRange("GameObjectProtoInstanceSpawnOffsetY", ref instance.m_minSpawnOffsetY, ref instance.m_maxSpawnOffsetY, -100, 100, showHelp);
                        editorUtils.SliderRange("GameObjectProtoInstanceSpawnOffsetZ", ref instance.m_minSpawnOffsetZ, ref instance.m_maxSpawnOffsetZ, -100, 100, showHelp);
                        //instance.m_rotateToSlope = editorUtils.Toggle("GameObjectProtoInstanceRotateToSlope", instance.m_rotateToSlope, showHelp);
                        ...
                        removeButtonRect.x += 15 * EditorGUI.indentLevel;
                        removeButtonRect.width -= 15 * EditorGUI.indentLevel;
                        if (GUI.Button(removeButtonRect, editorUtils.GetContent("GeNaSpawnerRemoveInstance")))
                        {
                            resourceProtoGeNaSpawner.m_instances = GaiaUtils.RemoveArrayIndexAt<ResourceProtoGeNaSpawnerInstance>(resourceProtoGeNaSpawner.m_instances, i);
                        }
                        EditorGUI.indentLevel--;
                    }
                }
                ...
                buttonRect.x += 15 * EditorGUI.indentLevel;
                buttonRect.width -= 15 * EditorGUI.indentLevel;
                if (GUI.Button(buttonRect, editorUtils.GetContent("GeNaSpawnerAddInstance")))
                {
                    resourceProtoGeNaSpawner.m_instances = GaiaUtils.AddElementToArray<ResourceProtoGeNaSpawnerInstance>(resourceProtoGeNaSpawner.m_instances, new ResourceProtoGeNaSpawnerInstance() { m_name = "New GeNa Spawner" });
                }
            }
            resourceProtoGeNaSpawner.m_dnaFoldedOut = editorUtils.Foldout("GameObjectProtoDNA", resourceProtoGeNaSpawner.m_dnaFoldedOut, showHelp);
            if (resourceProtoGeNaSpawner.m_dnaFoldedOut)
            {
                resourceProtoGeNaSpawner.m_dna.m_boundsRadius = editorUtils.FloatField("GameObjectProtoDNABoundsRadius", resourceProtoGeNaSpawner.m_dna.m_boundsRadius, showHelp);
            }
            EditorGUI.indentLevel--;
        }

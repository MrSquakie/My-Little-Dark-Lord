    ...
    public class SpawnerEditor : PWEditor, IPWEditor
    {
        ...
        private void DrawSpawnRuleMaskListElement(Rect rect, int index, bool isActive, bool isFocused)
        {
            if (m_maskListBeingDrawn != null && m_maskListBeingDrawn.Length > index)
            {
                ...
                ImageMaskListEditor.DrawMaskListElement(rect, index, m_maskListBeingDrawn[index], ref m_collisionMaskListBeingDrawn, m_editorUtils, Terrain.activeTerrain, false);
            }
        }
        ...
        {
            ImageMaskListEditor.DrawMaskListElement(rect, index, m_spawner.m_settings.m_imageMasks[index], ref m_collisionMaskListBeingDrawn, m_editorUtils, Terrain.activeTerrain,false);
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                //foreach (SpawnRule setDirtyRule in m_spawner.m_spawnerRules)
                //{
                //    //setDirtyRule.m_resourceIdx
                //    ImageMask[] masks = m_spawner.GetSpawnRuleImageMasksByIndex(setDirtyRule.m_resourceIdx);
                //    foreach (ImageMask mask in masks)
                //    {
                //        EditorUtility.SetDirty(mask.m_noiseSettings);
                //    }
        ...
        private bool DrawSingleRulePanel(SpawnRule rule, int ruleIdx)
        {
            ...
            GUILayout.BeginVertical(panelFrameStyle);
            {
                GUILayout.BeginHorizontal(m_singleSpawnRuleHeader);
                {
                    ...
                    GUILayout.FlexibleSpace();
                    DrawVisualiseButton(ruleIdx);
                    if (m_editorUtils.Button("X",GUILayout.Height(15)))
                    {
        ...
        private void DrawVisualiseButton( int spawnRuleID)
        {
            ...
            if (m_editorUtils.Button("Visualise", GUILayout.Height(15)))
            {
        ...
        void CleanUpRules()
        {
            ...
            while (idx < m_spawner.m_settings.m_spawnerRules.Count)
            {
                ...
                switch (rule.m_resourceType)
                {
                    ...
                    default:
                        {
                            if (rule.m_resourceIdx >= m_spawner.m_settings.m_resources.m_stampPrototypes.Length)
                            {

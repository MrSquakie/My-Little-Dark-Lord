    ...
    public class GaiaManagerEditor : EditorWindow, IPWEditor
    {
        ...
        void OnEnable()
        {
            ...
            var mainTabs = new Tab[] {
                ...
                new Tab ("Setup", m_gxIcon, SetupTab),
        ...
        void StandardTab()
        {
            ...
            m_editorUtils.Panel("NewWorldSettings", NewWorldSettings, true);
            ...
            if (m_editorUtils.ButtonAutoIndent("1. Create Terrains & Show Tools"))
            {
                ...
                if (actualTerrainCount != 0)
                {
                    ...
                    if (EditorUtility.DisplayDialog(m_editorUtils.GetTextValue("AlreadyFoundTerrainHeader"), string.Format(m_editorUtils.GetTextValue("AlreadyFoundTerrain"), actualTerrainCount, 0), m_editorUtils.GetTextValue("OK"), m_editorUtils.GetTextValue("Cancel")))
                    {
                        ...
                        GaiaDefaults.GetPrototypes(selectedPresetEntry.biomePreset.m_spawnerPresetList, ref terrainLayers, ref terrainDetails, ref terrainTrees, Terrain.activeTerrain);
...
#region Old Spawner Setup
        ...
        private void AdvancedPanelBiomesAndSpawners(bool helpEnabled)
        {
            ...
            {
                if (m_allBiomePresets.Exists(x => x.biomePreset == null))
                {
                    CreateAdvancedTabBiomesList();
                }
        ...
        void SetupTab()
        {
            //Show the panel settings
            m_editorUtils.Panel("PanelSetup", SetupSettingsEnabled, true);
        }
        ...
        private bool ExtrasSettingsEnabled(Rect rect, bool helpEnabled)
        {
            ...
            m_settings.m_currentWaterPro = (GaiaConstants.GaiaWaterProfileType)EditorGUI.EnumPopup(fieldRect, m_settings.m_currentWaterPro);
            if (m_settings.m_currentWaterPro != GaiaConstants.GaiaWaterProfileType.None)
            {
                labelRect.y += EditorGUIUtility.singleLineHeight;
                fieldRect.y += EditorGUIUtility.singleLineHeight;
            ...
            if (m_settings.m_createWind)
            {
                ...
                m_settings.m_windType = (GaiaConstants.GaiaWindType)EditorGUI.EnumPopup(fieldRect, m_settings.m_windType);
            }
        ...
        GameObject CreateWindZone()
        {
            ...
            if (windZoneObj == null)
            {
                ...
                {
                    case GaiaConstants.GaiaWindType.Calm:
                        windZone.windMain = 0.2f;
                        windZone.windTurbulence = 0.2f;
                        windZone.windPulseMagnitude = 0.2f;
                        windZone.windPulseFrequency = 0.05f;
                        break;
                    case GaiaConstants.GaiaWindType.Moderate:
                        windZone.windMain = 0.45f;
                        windZone.windTurbulence = 0.35f;
                        windZone.windPulseMagnitude = 0.2f;
                        windZone.windPulseFrequency = 0.1f;
                        break;
                    case GaiaConstants.GaiaWindType.Strong:
                        windZone.windMain = 0.65f;
                        windZone.windTurbulence = 0.3f;
                        windZone.windPulseMagnitude = 0.2f;
                        windZone.windPulseFrequency = 0.25f;
                        break;
                    case GaiaConstants.GaiaWindType.None:
            ...
            {
                WindZone windZone = windZoneObj.GetComponent<WindZone>();
                switch (m_settings.m_windType)
                {
                    case GaiaConstants.GaiaWindType.Calm:
                        windZone.windMain = 0.2f;
            ...
            GaiaLighting.AddGlobalWindShader();
        ...
        GameObject CreateWater()
        {
            ...
            {
                GaiaWater.GetProfile(m_settings.m_currentWaterPro, m_settings.m_gaiaWaterProfile);
            }
#endif
        ...
        {
            bool disablePackageButton = false;
            ...
            EditorGUILayout.BeginHorizontal();
            m_editorUtils.Label("UnityEngineVersion");
            EditorGUILayout.LabelField(Application.unityVersion, GUILayout.ExpandWidth(true));
            EditorGUILayout.EndHorizontal();
            ...
            if (m_settings.m_pipelineProfile.m_activePipelineInstalled != m_settings.m_currentRenderer)
            {
                ...
                {
                    disablePackageButton = true;
                    if (m_settings.m_currentRenderer == GaiaConstants.EnvironmentRenderer.Lightweight)
                    {
                        EditorGUILayout.BeginHorizontal();
                        m_editorUtils.LabelField("MinLWRP");
                        EditorGUILayout.LabelField(m_settings.m_pipelineProfile.m_minLWVersion);
                        EditorGUILayout.EndHorizontal();
                        EditorGUILayout.BeginHorizontal();
                        m_editorUtils.LabelField("MaxLWRP");
                        EditorGUILayout.LabelField(m_settings.m_pipelineProfile.m_maxLWVersion);
                        EditorGUILayout.EndHorizontal();
                    }
                    else if (m_settings.m_currentRenderer == GaiaConstants.EnvironmentRenderer.HighDefinition)
                    {
                        ...
                        EditorGUILayout.LabelField(m_settings.m_pipelineProfile.m_minHDVersion);
                        EditorGUILayout.EndHorizontal();
                        EditorGUILayout.BeginHorizontal();
                        m_editorUtils.LabelField("MaxHDRP");
                        EditorGUILayout.LabelField(m_settings.m_pipelineProfile.m_maxHDVersion);
                        EditorGUILayout.EndHorizontal();
                    }

                    if (m_editorUtils.Button("UpgradeGaiaTo" + m_settings.m_currentRenderer.ToString()))
                    {
                        if (EditorUtility.DisplayDialog("Upgrading Gaia Pipeline", "You're about to revert gaia back to use " + m_settings.m_currentRenderer.ToString() + " render pipeline. Are you sure you want to proceed?", "Yes", "No"))
                        {
            ...
            string vegetationPackage = "";
            string waterPackage = "";
            if (DoesPackagesNeedInstalled())
            {
                if (!disablePackageButton)
                {
                    GUILayout.Space(5f);
                    ...
                    {
                        GetPackages(GaiaPackageVersion.Unity2019_1, out waterPackage, out vegetationPackage);
                    }
                    else if (Application.unityVersion.Contains("2019.2"))
                    {
                        GetPackages(GaiaPackageVersion.Unity2019_2, out waterPackage, out vegetationPackage);
                    }
                    else if (Application.unityVersion.Contains("2019.3"))
                    {
                        GetPackages(GaiaPackageVersion.Unity2019_3, out waterPackage, out vegetationPackage);
                    }

                    EditorGUILayout.BeginHorizontal();
                    m_editorUtils.Label("WaterPackage");
                    if (m_settings.m_currentRenderer == GaiaConstants.EnvironmentRenderer.BuiltIn)
                    {
                        EditorGUILayout.LabelField(waterPackage);
                    }
                    EditorGUILayout.EndHorizontal();

                    EditorGUILayout.BeginHorizontal();
                    m_editorUtils.Label("VegetationPackage");
                    if (m_settings.m_currentRenderer == GaiaConstants.EnvironmentRenderer.BuiltIn)
                    {
                        EditorGUILayout.LabelField(vegetationPackage);
                    }
                    EditorGUILayout.EndHorizontal();
                }
                ...
                {
                    ProcedualWorlds.Gaia.PackageSystem.PackageInstallerUtils.m_installPackage1 = true;
                    ProcedualWorlds.Gaia.PackageSystem.PackageInstallerUtils.m_timer = 7f;
                    ProcedualWorlds.Gaia.PackageSystem.PackageInstallerUtils.StartInstallation(waterPackage, vegetationPackage);
                }
        ...
        /// </summary>
        /// <returns></returns>
        private bool DoesPackagesNeedInstalled()
        {
            bool doesNeedInstalling = false;
            if (Application.unityVersion.Contains("2018"))
            {
                Debug.LogError("Gaia 2/ Gaia 2 Pro does not work in 2018 please use 2019.1 or higher");
                return doesNeedInstalling;
            }
            else if (Application.unityVersion.Contains("2019.1"))
            {
                //Set true to test button
                doesNeedInstalling = true;
                //Check 2019.1 shaders
            }
            else if (Application.unityVersion.Contains("2019.2"))
            {
                //Check 2019.2 shaders
            }
            else if (Application.unityVersion.Contains("2019.3"))
            {
                //Check 2019.3 shaders
            }
        ...
        /// <summary>
        /// Gets the unity packages
        /// </summary>
        /// <param name="unityVersion"></param>
        /// <param name="waterPackage"></param>
        /// <param name="vegetationPackage"></param>
        private void GetPackages (GaiaPackageVersion unityVersion, out string waterPackage, out string vegetationPackage)
        {
            waterPackage = "";
            vegetationPackage = "";
            if (m_settings == null)
            ...
            foreach(GaiaPackageSettings packageSettings in m_gaiaPipelineSettings.m_packageSetupSettings)
            {
                if (packageSettings.m_unityVersion == unityVersion)
                {
                    if (m_settings.m_pipelineProfile.m_activePipelineInstalled == GaiaConstants.EnvironmentRenderer.BuiltIn)
                    {
                        waterPackage = packageSettings.m_builtInWaterPackage;
                        vegetationPackage = packageSettings.m_builtInVegetationPackage;
                    }
                    else if (m_settings.m_pipelineProfile.m_activePipelineInstalled == GaiaConstants.EnvironmentRenderer.Lightweight)
                    {
                        waterPackage = packageSettings.m_lightweightWaterPackage;
                        vegetationPackage = packageSettings.m_lightweightVegetationPackage;
                    }
                    else
                    {
                        waterPackage = packageSettings.m_highDefinitionWaterPackage;
                        vegetationPackage = packageSettings.m_highDefinitionVegetationPackage;
                    }                  
                }                
            }

...
namespace Gaia
{
    ...
    public class Spawner : MonoBehaviour
    {
        ...
        public GaiaHierarchyUtils m_hierarchyUtils;
        ...
        public int[] GetMissingResources()
        {
            ...
            for (int ruleIdx = 0; ruleIdx < m_settings.m_spawnerRules.Count; ruleIdx++)
            {
                if (m_settings.m_spawnerRules[ruleIdx].m_isActive == true) //Only care about active resources
                {
                    ...
                    else if (!m_settings.m_spawnerRules[ruleIdx].ResourceIsLoadedInTerrain(this))
                    {
                        ...
                        if (m_settings.m_spawnerRules[i].m_isActive)
                        {
                            ...
                            switch (m_settings.m_spawnerRules[i].m_resourceType)
                            {
                                ...
                                case GaiaConstants.SpawnerResourceType.TerrainDetail:
                                    ...
                                    if (detailLayerID != -1)
                                    {
                                        operation.SetTerrainDetails(ApplyBrush(operation, MultiTerrainOperationType.TerrainDetail, i), detailLayerID,  m_settings.spawnMode, m_settings.m_spawnerRules[i].m_terrainDetailDensity, ref m_settings.m_spawnerRules[i].m_spawnedInstances, false);
                                    }
                                    else
                                    {
                                ...
                                case GaiaConstants.SpawnerResourceType.GameObject:
                                    ...
                                    if (target == null)
                                    {
                                        Debug.LogWarning("Could not find or create spawn target transform for Spawn Rule " + m_settings.m_spawnerRules[i].m_name);
                                            break;
                                    }
                                    if (protoGO == null)
                                    {
                                ...
                                //    ResourceProtoGeNaSpawner protoSpawner = m_settings.m_resources.m_geNaSpawnerPrototypes[m_settings.m_spawnerRules[i].m_resourceIdx];
                                //    //List<GeNa.Spawner> geNaSpawners = new List<GeNa.Spawner>();

                                //    //Transform target = GetGOSpawnTarget(m_settings.m_spawnerRules[i], protoSpawner);
                                //    //if (target == null)
                                //    //{
                                //    //    Debug.LogWarning("Could not find or create spawn target transform for Spawn Rule " + m_settings.m_spawnerRules[i].m_name);
                                //    //    break;
                                ...
                                //    //    break;
                                //    //}
                                //    //operation.SetTerrainGameObjects(ApplyBrush(operation, MultiTerrainOperationType.GameObject, i), protoGO, m_settings.m_spawnerRules[i], transform, m_settings.spawnMode, ref m_settings.m_spawnerRules[i].m_spawnedInstances, m_settings.m_spawnerRules[i].m_minRequiredFitness, false);
                                //    break;
                            }
        ...
        public Transform GetGOSpawnTarget(SpawnRule rule, ResourceProtoGameObject protoGO)
        {
            if (rule.m_goSpawnTarget == null)
            {
                ...
                m_hierarchyUtils = parentTransform.GetComponent<GaiaHierarchyUtils>();
                if (m_hierarchyUtils == null)
                {
                    m_hierarchyUtils = parentTransform.gameObject.AddComponent<GaiaHierarchyUtils>();
                }
                m_hierarchyUtils.m_hideAllParentsInHierarchy = m_gaiaSettings.m_hideObjectsInHierachy;
...
#region Handy helpers
        ...
        public void ClearAllGameObjects()
        {
            foreach (SpawnRule spawnRule in m_settings.m_spawnerRules)
            {
                ...
                {
                    ClearGameObjectsForRule(spawnRule);
                }
            }
        }
        ...
        /// </summary>
        public void ClearGameObjectsForRule(SpawnRule spawnRule)
        {
            //Update the session
            ...
            Transform target = GetGOSpawnTarget(spawnRule, protoGO);

            if (target == null)
            {
                Debug.LogError("Could not find spawn target transform when trying to delete Game Objects from rule " + spawnRule.m_name);
                return;
            }

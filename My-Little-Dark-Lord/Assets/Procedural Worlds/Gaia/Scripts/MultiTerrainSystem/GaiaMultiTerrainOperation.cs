...
namespace Gaia
{
    ...
    public class GaiaMultiTerrainOperation
    {
        ...
        public void SetTerrainDetails(RenderTexture targetDetailTexture, int terrainLayerID, SpawnMode spawnMode, ref int instanceCounter, bool centerTerrainOnly = false)
        {
            ...
            foreach (var entry in relevantEntries)
            {
                ...
                for (int x = entry.Value.affectedLocalPixels.x; x < maxX; x++)
                {
                    for (int y = entry.Value.affectedLocalPixels.y; y < maxY; y++)
                    {
                        ...
                        map[y, x] = Mathf.Clamp(map[y, x] + invert * (UnityEngine.Random.value < (Mathf.Pow(colors[colorIndex].r, 5f)) ? (int)(colors[colorIndex].r * 15) : 0), 0, 15);
                                ...
                                if (UnityEngine.Random.value < Mathf.Pow(strength, 5f))
                                {
                                    ...
                                    switch (protoTree.m_spawnScale)
                                    {
                                        ...
                                        case SpawnScale.Random:
                                            treeInstance.widthScale = UnityEngine.Random.Range(protoTree.m_minWidth, protoTree.m_maxWidth);
                                            treeInstance.heightScale = UnityEngine.Random.Range(protoTree.m_minHeight, protoTree.m_maxHeight);
                                            break;
        ...
        public void GetCollisionMask(CollisionMask[] collisionMasks)
        {
            ...
            foreach (var entry in relevantEntries)
            {
                ...
                foreach (CollisionMask collMask in collisionMasks.Where(x => x.m_active == true))
                {
                    ...
                    if (tempsourceTexture != null)
                    {
                        ...
                        Graphics.Blit(completeCollisionMask, collisionMaskBuffer);
                        //ImageProcessing.DebugWriteRenderTexture("D:\\Tree1.png", bufferTexture);
                    }
                }
                if (tempsourceTexture != null)
                {
                    ...
                    oldFilterMode = tempsourceTexture.filterMode;
                    tempsourceTexture.filterMode = FilterMode.Point;
                    blitMaterial.SetTexture("_MainTex", finalTexture);
                    blitMaterial.SetPass(0);
                    CopyIntoPixels(entry.Value.affectedOperationPixels, entry.Value.affectedLocalPixels, finalTexture);
                    tempsourceTexture.filterMode = oldFilterMode;
                }
            ...
            RenderTexture.active = currentRT;
            if (tempsourceTexture != Texture2D.whiteTexture)
            {
                //Do not destroy the tempsourceTexture, as this will also destroy it in the collision mask cache in the session!
        ...
        private void AddAffectedTerrainPixels(RectInt opRect, MultiTerrainOperationType opType, int opWidth, int opHeight, TerrainLayer layer = null)
        {
            ...
            Bounds brushBounds = new Bounds(m_originTransform.position, new Vector3(m_range, m_originTerrain.terrainData.size.y, m_range));

namespace ProcedualWorlds.WaterSystem.MeshGeneration
{
    ...
    public class PWS_MeshGenerationPro : MonoBehaviour
    {
        ...
        /// </summary>
        [HideInInspector]
        public Vector2 m_meshPoints = new Vector2(50, 50);
        /// <summary>
        /// Mesh Type in use
        /// </summary>
        [HideInInspector]
        ...
        public void SizeGeneration()
        {
            if (m_MeshType == MeshType.Plane)
            {
                m_uvScale = new Vector2(1f / (m_meshPoints.x), 1f / (m_meshPoints.y));
                m_sizeScale = new Vector3(m_Size.x / (m_meshPoints.x), m_Size.y, m_Size.z / (m_meshPoints.y));
            }
            if (m_MeshType == MeshType.Circle)
            {
                float half = m_Size.x / 2;
                m_uvScale = new Vector2(1f / (m_meshPoints.x), 1f / (m_meshPoints.x));
                m_sizeScale = new Vector3(half / (m_meshPoints.x), m_Size.y, half / (m_meshPoints.x));
            }
        }
        /// <summary>
        ...
        public void VerticesGeneration()
        {
            ...
            {
                Vector3 MiddlePoint;
                MiddlePoint = new Vector3(m_meshPoints.x / 2, 0, m_meshPoints.y / 2);
                for (int y = 0; y < (int)m_meshPoints.y; ++y)
                {
                    for (int x = 0; x < (int)m_meshPoints.x; ++x)
                    {
            ...
            if (m_MeshType == MeshType.Circle)
            {
                ...
                for (int CirclePoint = 0; CirclePoint < m_meshPoints.x; ++CirclePoint)
                {
        ...
        public void TriangleGeneration()
        {
            ...
            {
                int x, y;
                for (y = 0; y < (int)m_meshPoints.y - 1; y++)
                {
                    for (x = 0; x < (int)m_meshPoints.x - 1; x++)
                    {
                        // For each grid cell output two triangles
                        m_triangles.Add((y * (int)m_meshPoints.x) + x);
                        m_triangles.Add(((y + 1) * (int)m_meshPoints.x) + x);
                        m_triangles.Add((y * (int)m_meshPoints.x) + x + 1);
                        m_triangles.Add(((y + 1) * (int)m_meshPoints.x) + x);
                        m_triangles.Add(((y + 1) * (int)m_meshPoints.x) + x + 1);
                        m_triangles.Add((y * (int)m_meshPoints.x) + x + 1);
                    }
                }
            ...
            {
                m_triangles.Clear();
                for (int circ = 0; circ < (int)m_meshPoints.x; ++circ)
                {
        ...
        public void UVGeneration()
        {
            if (m_MeshType == MeshType.Plane)
            {
                Vector2 MiddlePoint = new Vector2(m_meshPoints.x / 2, m_meshPoints.y / 2);
                for (int y = 0; y < (int)m_meshPoints.y; ++y)
                {
                    for (int x = 0; x < (int)m_meshPoints.x; ++x)
                    {
        ...
        /// </summary>
        /// <returns>total vertice count</returns>
        public int CalculateVerticesRequired()
        {
            if (m_MeshType == MeshType.Plane)
            {
                return (int)m_meshPoints.y * (int)m_meshPoints.x;
            }
            if (m_MeshType == MeshType.Circle)
            {
                return (int)m_meshPoints.x * ((int)m_meshPoints.x) * 6;
            }
            return 0;
        }
        #endregion

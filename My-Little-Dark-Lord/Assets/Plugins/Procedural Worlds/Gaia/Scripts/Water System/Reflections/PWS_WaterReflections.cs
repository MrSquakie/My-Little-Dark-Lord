    ...
    public class PWS_WaterReflections : MonoBehaviour
    {
        ...
        public void OnWillRenderObject()
        {
            ...
            if (!EditorApplication.isPlaying)
            {
                ...
                if (m_RenderCamera != null)
                {
                    if (m_RenderCamera.transform.hasChanged)
                    {
                        CameraSetup();
                        GenerateCamera();
        ...
        /// </summary>
        /// <param name="Value">Pixel Light value</param>
        public void RenderShadows(bool Value)
        {
            if (Value)
            {
                if (QualitySettings.shadowDistance != m_oldShadowDistance)
                {
                    QualitySettings.shadowDistance = m_oldShadowDistance;
                }
            }
            else
            {
                m_oldShadowDistance = QualitySettings.shadowDistance;
        ...
        public void ResyncCameraSettings()
        {
            ...
            {
                m_reflectionCamera.farClipPlane = m_renderDistance;
            }
            else
            {
                m_reflectionCamera.farClipPlane = m_RenderCamera.farClipPlane;
            }
            m_reflectionCamera.nearClipPlane = m_RenderCamera.nearClipPlane;
            ...
            // ReflectionCamera.renderingPath = m_RenderPath;
        ...
        public void GenerateReflection()
        {
            ...
            {
                m_reflectionCamera.cullingMask = ~(1 << 4) & m_reflectionLayers.value; // never render water layer
            }

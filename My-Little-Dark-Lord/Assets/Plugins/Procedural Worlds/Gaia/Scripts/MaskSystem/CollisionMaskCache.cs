...
namespace Gaia
{
    ...
    public class CollisionMaskCache : UnityEngine.Object
    {
        ...
        public void BakeTerrainTagCollisions(Terrain terrain, string tag, float radius)
        {
            ...
            BakeVectorArrayForTerrain(posAndRad, terrain, fileName);
        }
        ...
        public void WriteCacheToDisk()
        {
            ...
            {
                ImageProcessing.WriteTexture2D(m_cacheEntries[i].fullPath, m_cacheEntries[i].texture);
        ...
        public void BakeTerrainTreeCollisions(Terrain terrain, int treePrototypeId, float boundsRadius)
        {
        ...
        private void BakeVectorArrayForTerrain(Vector4[] posAndRad, Terrain terrain, string filename)
        {
            ...
            RenderTexture collisionMask = RenderTexture.GetTemporary(rtDesc);
        ...
        private void SaveCollisionMaskInCache(RenderTexture renderTexture, Terrain terrain, string fileName)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                if (m_cacheEntries[i].fileName == fileName)
                {
                    ...
                    found = true;
                    WriteCollisionCacheEntry(m_cacheEntries[i], texture, terrain, fileName);
                    break;
                }
            }
            ...
            if (!found)
            {
                AddNewCollisionMaskCacheEntry(texture, terrain, fileName);
            }


        }

        private void AddNewCollisionMaskCacheEntry(Texture2D texture, Terrain terrain, string fileName)
        {
        ...
        private void WriteCollisionCacheEntry(CollisionMaskCacheEntry entry, Texture2D texture, Terrain terrain, string fileName)
        {
        ...
        public Texture2D LoadCollisionMask(Terrain terrain, CollisionMaskType type, float radius, int id = 0, string tag = "")
        {

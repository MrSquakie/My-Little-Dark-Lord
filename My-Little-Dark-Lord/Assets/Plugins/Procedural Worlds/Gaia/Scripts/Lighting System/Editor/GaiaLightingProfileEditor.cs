...
namespace Gaia
{
    ...
    public class GaiaLightingProfileEditor : PWEditor
    {
        ...
        private GaiaConstants.GaiaLightingProfileType m_lightingProfile;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                {
                    if (m_lightingProfile != GaiaConstants.GaiaLightingProfileType.None)
                    {
                        GaiaLighting.GetProfile(m_lightingProfile, m_profile, m_gaiaSettings.m_pipelineProfile, m_gaiaSettings.m_pipelineProfile.m_activePipelineInstalled);
                    }
                }
            }
        }
        ...
        {
            m_profile.m_updateInRealtime = m_editorUtils.ToggleLeft("UpdateChangesInRealtime", m_profile.m_updateInRealtime);
            if (m_profile.m_updateInRealtime)
            {
        ...
        {
            m_lightingProfile = (GaiaConstants.GaiaLightingProfileType)EditorGUILayout.EnumPopup("Time Of Day Profile", m_lightingProfile);
            GUILayout.Space(10f);
            //Profile values
            ...
            else if (m_lightingProfile != m_profileValues.m_profileType)
            {
                m_profileValues = GetProfile();
            }
            ...
            else
            {
                if (m_renderPipeline != GaiaConstants.EnvironmentRenderer.HighDefinition)
                {
                    ...
                    if (m_lightingProfile == GaiaConstants.GaiaLightingProfileType.Default)
                    {
        ...
        private GaiaLightingProfileValues GetProfile()
        {
            ...
            {
                if (profile.m_profileType == m_lightingProfile)
                {
                    return profile;
                }
            }

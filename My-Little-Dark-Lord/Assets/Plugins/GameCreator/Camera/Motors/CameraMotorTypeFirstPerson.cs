        ...
        private void RotationInput()
        {
            if (Application.isMobilePlatform)
            {
                ...
                for (int i = 0; i < Input.touchCount; ++i)
                {
                    ...
                    {
                        this.targetRotationX += touch.deltaPosition.x / Screen.width * this.sensitivity.x * 10f * Time.timeScale;
                        this.targetRotationY += touch.deltaPosition.y / Screen.height * this.sensitivity.y * 10f * Time.timeScale;
                        break;
                    }
                }
            }
            else
            {
                ...
                if (inputConditions)
                {
                    this.targetRotationX += Input.GetAxis(INPUT_MOUSE_X) * this.sensitivity.x * Time.timeScale;
                    this.targetRotationY += Input.GetAxis(INPUT_MOUSE_Y) * this.sensitivity.y * Time.timeScale;
                }
            }

namespace GameCreator.Characters
{
    ...
    public class CharacterRagdoll
    {
        ...
        private class HumanChunk
        {
            ...
            public HumanChunk(Transform transform, bool useLocal)
            {
                this.transform = transform;
                this.Snapshot(false);
            }

            public void Snapshot(bool useLocal)
            {
                if (useLocal)
                {
        ...
        private void ToRecover()
        {
            ...
            {
                case true:
                    this.charAnimator.PlayGesture(this.charAnimator.standFaceDown, 1f, null, 0f, 0.25f);
                    ragdollDirection = this.GetRagdollDirection();
                    break;

                case false:
                    this.charAnimator.PlayGesture(this.charAnimator.standFaceUp, 1f, null, 0f, 0.25f);
        ...
        private Vector3 GetRagdollDirection()
        {
            ...
            return Vector3.Scale((pointHead - pointFeet), PLANE_XZ);
        }
        ...
        private bool BuildChunks()
        {
            ...
            this.rootChunk = new HumanChunk(target, true);
            this.chunks = new List<HumanChunk>();
            for (int i = 0; i < children.Length; ++i)
            {
                if (children[i] == target) continue;
                this.chunks.Add(new HumanChunk(children[i], false));
            }

...
namespace Invector.vEventSystems
{
    public class vAnimatorTagAdvanced : vAnimatorTagBase
    {
        ...
        public class vAdvancedTags
        {
            ...
            bool isEnter;
            bool isExit;
            public void UpdateEventTrigger(float normalizedTime, List<vAnimatorStateInfos> stateInfos, int layer, float speed = 1, bool debug = false)
            {
                var normalizedTimeClamped = Mathf.Clamp(normalizedTime, 0, loopCount + 1f);
                if (!isEnter && tagType != vAnimatorEventTriggerType.EnterStateExitByNormalized &&
                ...
                if (!isExit && isEnter && tagType != vAnimatorEventTriggerType.EnterByNormalizedExitState &&
                                               tagType != vAnimatorEventTriggerType.EnterStateExitState && normalizedTimeClamped >= loopCount + (this.normalizedTime.y / speed))
                {
        ...
        public override void OnStateExit(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
        {
            if (stateInfos != null)
            {
                for (int i = 0; i < tags.Count; i++)
                {
                    ...
                    else tags[i].UpdateEventTrigger(stateInfo.normalizedTime, stateInfos, layerIndex, animator.speed, debug);
                }
            }

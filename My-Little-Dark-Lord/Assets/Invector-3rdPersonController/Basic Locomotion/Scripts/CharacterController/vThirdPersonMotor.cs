...
namespace Invector.vCharacterController
{
    public abstract class vThirdPersonMotor : vCharacter
    {
        ...
        [Range(0f, 4f), Tooltip("To Disable StepOffset, set value to Zero.\nHigher value will result jittering on ramps, lower values will have difficulty on steps")]
        ...
        internal float verticalVelocity;    // general variables to the locomotion
                                            // velocity to apply to rigdibody
        ...
        /// <summary>
        /// Change  the value without smooth
        /// </summary>
        internal float _currentSpeed, _currentDirection, _currentVelocity;
        /// <summary>
        /// Change the value with smooth <seealso cref="vMovementSpeed.acceleration"/>
        /// </summary>
        internal float speed
        {
            ...
            set
            {
                _currentSpeed = Mathf.Lerp(_currentSpeed, value, (isStrafing ? strafeSpeed.acceleration : freeSpeed.acceleration * Time.deltaTime));
            }
        }
        /// <summary>
        /// Change the value with smooth <seealso cref="vMovementSpeed.acceleration"/>
        ...
        internal float direction
        {
            ...
            set
            {
                _currentDirection = Mathf.Lerp(_currentDirection, value, (isStrafing ? strafeSpeed.acceleration : freeSpeed.acceleration * Time.deltaTime));
            }
        }
        /// <summary>
        /// Change the value with smooth <seealso cref="vMovementSpeed.acceleration"/>
        ...
        internal float velocity
        {
            ...
            set
            {
                _currentVelocity = Mathf.Lerp(_currentVelocity, value, (isStrafing ? strafeSpeed.acceleration : freeSpeed.acceleration * Time.deltaTime));
            }
        }
        #endregion
        ...
        public virtual void ControlLocomotion()
        {
            ...
            {
                StrafeMovement();
                // Strafe Rotation uses the methods RotateToTarget and RotateToDirection 
            }
        }
        ...
        public virtual void FreeMovement()
        {
            ...
            var _speed = Mathf.Abs(input.x) + Mathf.Abs(input.y);
            // limits the character to walk by default
            if (freeSpeed.walkByDefault)
                _speed = Mathf.Clamp(_speed, 0, 0.5f);
            else
                _speed = Mathf.Clamp(_speed, 0, 1f);
            // add 0.5f on sprint to change the animation on animator
            if (isSprinting) _speed += 0.5f;
            if (stopMove || lockSpeed) _speed = 0f;
            speed = _speed;
        }

        public virtual void FreeRotation()
        {
            if (lockRotation || (customAction && !isRolling)) return;

            //var conditions = (isRolling && rollControl);
            if (input != Vector2.zero && targetDirection.magnitude > 0.1f)
            {
                ...
                if (isGrounded || (!isGrounded && jumpAirControl))
                {
                    ...
                    if (animator.IsInTransition(0) && inTurn || isRolling && !rollControl) return;
        ...
        public virtual void ControlSpeed(float velocity)
        {
            ...
            if (useRootMotion && !customAction && !forceRootMotion)
            {
            ...
            // use only RootMotion 
            else if (customAction || forceRootMotion)
            {
                this.velocity = 0;
            ...
            else
            {
                if (input.magnitude > 0.1f || isJumping || isSliding)
                {
                    this.velocity = velocity;
                    if (isStrafing)
                        StrafeVelocity(_currentVelocity);
                    else
                        FreeVelocity(_currentVelocity);
                }
                else
                {
                    this.velocity = 0;
        ...
        {
            if (input.sqrMagnitude < 0.1 || !isGrounded || stopMove || isSliding || isJumping || stepSmooth <= 0) return false;
            ...
            if (Physics.Raycast(rayStep, out _hit, stepOffsetEnd - stepOffsetStart, groundLayer) && !_hit.collider.isTrigger)
            {
                if (_hit.point.y >= (transform.position.y + 0.1f) && _hit.point.y <= (transform.position.y + stepOffsetEnd))
                {
                    ...
                    var vel = _rigidbody.velocity;
                    vel.y = (velocityDirection * stepSmooth * ((velocity > 1 ? velocity : 1))).y;
                    _capsuleCollider.material = slippyPhysics;
                    _rigidbody.velocity = vel;
                    return true;
                }
        ...
        {
            if (lockRotation || customAction) return;
            if (target)
            {
        ...
        {
            [Tooltip("Smooth acceleration between walk, running and sprinting speeds")]
